...
var resizePizzas = function(size) {
  ...
  // #Performance 1: Moved code out of the loop and store the container object and length
  // in a vairable so that the dom is not parsed every time.
...
// ##Performance 2: Initialize new variables to store current scroll position
// and a boolean variable to check scrolling or not
var currentScrollPositionY = 0;
var isScrolling = true;

// ##Performance 2: Created a  new function which will be called on scroll
function onScroll() {
  //Update the window scroll y position
  currentScrollPositionY = window.scrollY;
  if (!isScrolling) {
    //Inform the browser that we are going to perform an animation
    // Pass the method to update the position
...
function updatePositions() {
  ...
  for (var i = 0; i < items.length; i++) {
    //#Performance 2: inserted currentScrollPositionY  which is calculated on scroll.
    var phase = Math.sin((currentScrollPositionY / 1250) + (i % 5));
...
document.addEventListener('DOMContentLoaded', function() {
 ...
 // Performance 6: Calculate background pizza image count based on the window height.
  var cols = 8;
  var s = 256;
  var bgPizzaCount = Math.ceil((window.innerHeight / s) * cols);
  for (var i = 0; i < bgPizzaCount; i++) {
